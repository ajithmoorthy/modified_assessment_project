...
public class ValidaterHelper extends Extent{
	...
	public void sample(WebDriver driver,String dateloc,String readloc,String blogtitleloc,String blogcontentloc,String bdateloc,String[] expectedtitle,String[] expectedcontent,String insightsloc,String blogsloc) throws ParseException {
		SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");
	    Date date=new Date();
		WebDriverWait wait2 = new WebDriverWait(driver, 20);
		...
		for(int count=0; count<readlist.size(); count++) {
			for(WebElement child:datelist) {
				System.out.println(child.getText());
				String[] tempdate=child.getText().split(" ");	
				String exam=tempdate[1].substring(0,tempdate[1].length()-1)+"/"+getMonthNumber(tempdate[0])+"/"+tempdate[2];
				exam=exam.replace("\\s", "");
				String secon=formatter.format(date);
				Date first=formatter.parse(exam);
				...
				if(numdays>90) {
					...
					assertValidate(title, expectedtitle[count]);
					assertValidate(blogdate,child.getText());
					assertValidate(content, expectedcontent[count]);
					helper.mouseOver(insightsloc, driver);
					helper.mouseOver(blogsloc, driver);
					helper.clickElement(blogsloc, driver);
					count++;
				}
			}
